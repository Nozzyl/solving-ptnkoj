...
bool fermatPrimalityTest(long long n, long long k = 30)
{
    if (n < 4)
        return n == 2 or n == 3;
...
int main() {
    long long n;
    std::cin >> n;
    // std::vector<bool> isPrime = sieveOfEratosthenes(n + 2);
    for (long long i = (long long) n / 2 + 1; i >= 1; i--) {
        if (fermatPrimalityTest(i) and fermatPrimalityTest(n - i)) {
            std::cout << i << " " << n - i;
            break;
        }
    }
